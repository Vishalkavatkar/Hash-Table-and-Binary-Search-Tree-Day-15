 ...
 *  Method add to add the node with key & value to the MapNode
 *
...
public class HashMap<K, V> {
	...
	public V get(K key) {
		 ...
		 * Then we pass the index number to the arrayList to get the array's LinkedList & save this to the variable LinkedList
		 * If LinkedList is null then we return null
		 * If present then we  search the key in the LinkedList and save the value and key on the maNode
		 * If mapNode is null then we return null or if value present then we return value 
	...
	public void add(K key, V value) {
		 ...
		 * First we find the indexNumber which will range in between the bucket number.
		 * Then we pass the index number to the arrayList to get the array's LinkedList & save this to the variable LinkedList
		 * If the arrayList is null then we create a new LinkedKist and set the LinkedList to that index
		 * If LinkedList is preset then we search for that Key
		 * If not preset then we make new node and append that to the LinkedList
		 * if key is present then we just update the value
